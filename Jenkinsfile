pipeline {
    ...
    stages {
        ...
        stage('ZAP Scan') {
            steps {
                sh '''
                # 1. Set working directory to /zap/wrk and run the scan
                docker exec -w /zap/wrk zap zap-baseline.py -t http://petclinic-staging:8080 \
                    -r zap_report.html -x zap_report.xml || true

                # 2. Copy the report from /zap/wrk/ into the Jenkins workspace
